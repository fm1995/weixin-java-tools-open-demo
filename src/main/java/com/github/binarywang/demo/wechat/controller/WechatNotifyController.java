...
//    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    ...
    public Object receiveTicket(@RequestBody(required = false) String requestBody, @RequestParam("timestamp") String timestamp,
        ...
        this.logger.info(
                "\n接收微信请求receive_ticket：[signature=[{}], encType=[{}], msgSignature=[{}],"
        ...
        if (!StringUtils.equalsIgnoreCase("aes", encType) || !wxOpenService.getWxOpenComponentService().checkSignature(timestamp, nonce, signature)) {
            throw new IllegalArgumentException("非法请求，可能属于伪造的请求！receive_ticket");
        }
    ...
    public Object callback(@RequestBody(required = false) String requestBody,
        ...
        this.logger.info(
                "\n接收微信请求callback：[appId=[{}], openid=[{}], signature=[{}], encType=[{}], msgSignature=[{}],"
                        + " timestamp=[{}], nonce=[{}], requestBody=[\n{}\n] ",
                appId, openid, signature, encType, msgSignature, timestamp, nonce, requestBody);
        if (!StringUtils.equalsIgnoreCase("aes", encType) || !wxOpenService.getWxOpenComponentService().checkSignature(timestamp, nonce, signature)) {
            throw new IllegalArgumentException("非法请求，可能属于伪造的请求callback！");
        }
    ...
    public String getuid() {
        try{
            WxMpUser wxMpUser = wxOpenService.getWxOpenComponentService().getWxMpServiceByAppid("wx407ff98f415361dc").getUserService().userInfo("otDARwHUnftwC-22qf9HRpZ4unYg");
